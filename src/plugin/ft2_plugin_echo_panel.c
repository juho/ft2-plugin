 ...
 * @brief Echo effect modal panel implementation
 */
...
typedef struct {
	...
	int16_t echoNum;       /* Number of echoes (0-64) */
	int32_t echoDistance;  /* Distance (0-16384, multiplied by 16 for actual samples) */
	int16_t echoVolChange; /* Volume change per echo (0-100%) */
	bool echoAddMemory;    /* Add memory to sample or not */
} echo_panel_state_t;
...
{
	(void)inst;
	state.echoNum = (int16_t)pos;
}

static void onEchoDistScrollbar(ft2_instance_t *inst, uint32_t pos)
{
	(void)inst;
	state.echoDistance = (int32_t)pos;
}

static void onEchoVolScrollbar(ft2_instance_t *inst, uint32_t pos)
{
	(void)inst;
	state.echoVolChange = (int16_t)pos;
}
...
static void drawFrame(ft2_video_t *video, const ft2_bmp_t *bmp)
{
	...
	if (state.echoAddMemory)
		charOut(video, bmp, 178, 270, PAL_FORGRND, 'x');
}
...
static void applyEchoToSample(void)
{
	...
	int32_t k = 0;
	while (k < state.echoNum && dSmp >= 1.0)
	{
		dSmp *= dVolChange;
		k++;
	}
	...
	int32_t dataLen = writeLen * bytesPerSample;
