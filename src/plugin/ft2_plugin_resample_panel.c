/**
 * @file ft2_plugin_resample_panel.c
 * @brief Resample modal panel implementation
 */
...
static resample_panel_state_t state = {
	.active = false,
	.instance = NULL,
	.relReSmp = 0
};
...
static void drawFrame(ft2_video_t *video, const ft2_bmp_t *bmp)
{
	...
	/* Calculate new sample length for display */
	int32_t newLen = 0;
	ft2_sample_t *s = getCurrentSample();
	if (s != NULL && s->dataPtr != NULL)
	{
		double dLenMul = pow(2.0, state.relReSmp * (1.0 / 12.0));
	...
	/* Draw relative halftones value */
	char sign;
	if (state.relReSmp == 0) sign = ' ';
	else if (state.relReSmp < 0) sign = '-';
	else sign = '+';
	...
	if (val > 9)
	{
		...
		charOut(video, bmp, 298, 236, PAL_FORGRND, '0' + ((val / 10) % 10));
